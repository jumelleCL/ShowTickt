...
class LlistatEntradesTest extends TestCase
{
    use RefreshDatabase;

    public function test_index_method_returns_view_with_entrades()
    {
        ...
        $response = $this->actingAs($user)->withSession(['key' => $user->id])->get('/llistats-entrades/'.$sessio1->id);
        $response->assertOk();
    }
}