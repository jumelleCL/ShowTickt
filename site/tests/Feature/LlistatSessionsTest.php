...
class LlistatSessionsTest extends TestCase
{
    use RefreshDatabase;

    public function test_index_method_returns_view_with_sessions()
    {
        ...
        $response = $this->actingAs($user)->withSession(['key' => $user->id, 'user_id' => $user->id])->post('llistat-sessions', $form);
