...
class EventDetailsTest extends TestCase
{
    ...
    public function test_index_method_returns_view_with_event_details()
    {
        ...
        $response = $this->actingAs($user)->withSession(['key' => $user->id])->get('/detalls-esdeveniment/'.$esdeveniment1->id);
        $response->assertOk();
    }
}