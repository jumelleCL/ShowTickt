...
class EventsAdministrarTest extends TestCase
{
    use RefreshDatabase;

    public function test_index_method_returns_view_with_administrar_event()
    {
        ...
        $response = $this->actingAs($user)->withSession(['key' => $user->id])->get('/administrar-esdeveniment/'.$esdeveniment1->id);
        $response->assertOk();
    }
}